...
- When passing arguments to long-form flags, use the space form (`--flag value`), not the equals form (`--flag=value`). Both are accepted by `getopt_long` for required arguments, so this is a style choice — pick the space form for visual consistency with short flags (`-n 1`) and to keep argument values free of inline `=` characters. Exceptions where the equals form is required: when the long flag's argument is *optional* (e.g. `xargs --replace=@`, since `getopt_long` only attaches optional values via `=`), and when the argument value itself begins with `-` (e.g. `--lines=-5`, to disambiguate from another flag).
