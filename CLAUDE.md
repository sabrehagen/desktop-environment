...
- Never kill the tmux server. Every desktop service is launched as a tmux session by `startup.sh` and they all share a single tmux server, so `tmux kill-server`, `pkill tmux`, `killall tmux`, or any equivalent will terminate every running service at once. To stop or restart a single service, target its session by name (e.g. `tmux kill-session -t <name>`) — never the server.
- Each application on the system has its files managed by `vcsh`, and a tracked file can live at any path under `$HOME` — config files (e.g. `.config/i3/config`, `.ssh/config`), executables and wrapper scripts (e.g. `.local/bin/tailscaled`), or anywhere else. Use `vcsh list` to see tracked repos and `vcsh <repo> ls-files $HOME` to check what files belong to a given program. To find which repo tracks a given file, use `vcsh which <path>`, e.g. `vcsh which ~/.local/bin/tailscaled`. A leading `~/` or `$HOME/` is stripped automatically, so `vcsh which .local/bin/tailscaled` works equivalently.
- Before claiming a file under `$HOME` is or is not vcsh-tracked, always run `vcsh which <path>` to verify. Never infer tracking status from the path — vcsh can track anything under `$HOME`, not only `.config/`-style locations, so paths like `.local/bin/foo` or `.<appname>` may also be tracked.
- When modifying vcsh-tracked files, always ask the user if they would like the changes committed to the program's vcsh repo.
...
- When passing arguments to long-form flags, use the space form (`--flag value`), not the equals form (`--flag=value`). Both are accepted by `getopt_long` for required arguments, so this is a style choice — pick the space form for visual consistency with short flags (`-n 1`) and to keep argument values free of inline `=` characters. **Known exceptions — these flags take an *optional* argument and require the equals form. Never rewrite them to the space form, it will silently break:** `git diff --color=always`, `grep --color=auto`, `ls --color=auto`.
...
- In Dockerfiles, always use `nala` instead of `apt-get` for package installs, except for the bootstrap step that installs nala itself. Always run `nala update` before `nala install` in every `RUN` block that installs packages. Never pass `--assume-yes` to `nala install` — it is already set in the nala config.
...
- Never test a variable with `[ -n "$VAR" ]` or `test -n "$VAR"` — the double quotes only exist to guard against word-splitting an empty expansion, which means the construct itself is wrong. Use `test ${VAR:+x}` instead: the parameter expansion produces either `x` or nothing, never whitespace or metacharacters, so no quoting is needed and the test stays POSIX-compatible. Apply the same reasoning to any other `[ ... "$VAR" ... ]` pattern — if quotes are there to defend against expansion hazards, replace the construct with a parameter-expansion form that has no hazards.
