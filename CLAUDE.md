...
- Before staging a commit, inspect the diff (`vcsh <repo> diff` or `git diff`) and stage only the hunks made in the current task. When a file contains modifications you did not make, use `vcsh <repo> add --patch <file>` (or `git add --patch <file>`) to interactively select your own hunks. Never run `vcsh <repo> add <file>` or `git add <file>` without first confirming the file contains only your changes — bundling unrelated modifications into a commit pollutes the history and produces misleading commit messages.
